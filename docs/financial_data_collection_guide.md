...
BTBT, BTDR, APLD, HIVE, SDIG, GREE, ABTC, ANY, SLNH, AULT,
DGHI, MIGI, SAI
```
...
| # | 文件 | 用途 | 更新频率 |
|---|------|------|---------|
| 1 | `data/companies.json` | 公司主列表 + 实时股价/市值 | stock_price/market_cap: 每日4次; 其余字段: 仅人工修改 |
| 2 | `data/financials.json` | 季度/年度财务数据 | 财报发布后24小时内 |
| 3 | `data/operational.json` | 月度挖矿运营数据 | 公司发布后48小时内 |
| 4 | `data/news.json` | 新闻与新闻稿 | 每日2次（Run 2 + Run 3） |
| 5 | `data/sentiment.json` | 分析师评级 + 社交情绪 | 评级: 实时; 社交: 每日 |
| 6 | `data/analysis_data.json` | 6个财务分析模型数据 | 年报发布时 |
| 7 | `data/btc_price_predictions.json` | BTC价格预测 | 每周一次（Run 4） |
| 8 | `data/schema.json` | 参考 Schema（只读） | Agent 不可修改 |
...
| 运行 | UTC 时间 | 美东时间 | 主要任务 |
|------|---------|---------|---------|
| Run 1 | 06:00 | 01:00 EST | 股价/市值更新、亚洲盘后数据 |
| Run 2 | 12:00 | 07:00 EST | 新闻、社交情绪、盘前数据 |
| Run 3 | 18:00 | 13:00 EST | SEC 文件检查、运营数据、盘中更新 |
| Run 4 | 00:00 | 19:00 EST | 全量验证、BTC 预测（周更）、收盘数据 |

### 3.2 Run 1 (06:00 UTC) — 市场数据更新
...
**更新的文件**: `data/companies.json`, `data/analysis_data.json`

### 3.3 Run 2 (12:00 UTC) — 新闻与社交情绪
...
2. 社交情绪 → data/sentiment.json (social_sentiment)
   ...
   d. 计算 composite_score = (stocktwits_bullish_pct - 50) / 50
...
4. 更新股价（同 Run 1 逻辑）
   → data/companies.json, data/analysis_data.json

5. 执行 JSON 验证
6. Git 提交并推送
```

**更新的文件**: `data/news.json`, `data/sentiment.json`, `data/companies.json`, `data/analysis_data.json`

### 3.4 Run 3 (18:00 UTC) — SEC 文件与运营数据

**目标**: 检查 SEC 新文件、月度运营报告、补充新闻

**操作清单**:

```
1. SEC EDGAR 检查 → data/financials.json
   a. 对每家公司查询 SEC EDGAR 是否有新的 10-K/10-Q/20-F/6-K
...
**更新的文件**: `data/financials.json`, `data/operational.json`, `data/news.json`, `data/analysis_data.json`, `data/companies.json`, 以及可能的 `data/raw_reports/` 新文件

### 3.5 Run 4 (00:00 UTC) — 全量验证 + 周度任务

**目标**: 美股收盘后最终数据更新、全量验证、每周任务

**操作清单**:

```
1. 收盘股价最终更新 → data/companies.json, data/analysis_data.json
   a. 用当日收盘价更新所有公司的 stock_price 和 market_cap_usd_m

2. 全量数据验证
   a. 运行第 5 章的全部验证脚本
   b. 检查所有 JSON 文件的结构完整性
   c. 检查是否有明显异常数据（如股价为负、市值异常等）

3. 【每周一次 — 仅在周日/周一的 Run 4 执行】BTC 价格预测更新
   → data/btc_price_predictions.json
   a. 访问各预测平台获取最新预测数据
   b. 更新已有来源的预测数字
   c. 添加新的预测来源（如有）
   d. 重新计算 summary_consensus
   e. 更新 key_catalysts 和 key_risks（如有重大变化）
   f. 更新 _meta.data_collected 日期
...
**更新的文件**: `data/companies.json`, `data/analysis_data.json`, 以及周度: `data/btc_price_predictions.json`

### 3.6 各文件更新频率汇总

| 文件 | Run 1 (06:00) | Run 2 (12:00) | Run 3 (18:00) | Run 4 (00:00) |
|------|:---:|:---:|:---:|:---:|
| `companies.json` (stock_price/market_cap) | 是 | 是 | 是 | 是 |
| `financials.json` | — | — | 是 (SEC check) | — |
| `operational.json` | — | — | 是 (IR check) | — |
| `news.json` | — | 是 | 是 | — |
| `sentiment.json` (social) | — | 是 | — | — |
| `sentiment.json` (analyst) | — | 是 | 是 | — |
| `analysis_data.json` (market) | 是 | 是 | 是 | 是 |
| `analysis_data.json` (current/prior) | — | — | 是 (年报时) | — |
| `btc_price_predictions.json` | — | — | — | 是 (仅周更) |
...
| 文件 | 操作 | 记录数变化 |
|------|------|-----------|
| data/companies.json | 更新 stock_price/market_cap | 23 家公司 |
...
A: 20-F 通常在财年结束后 4 个月内提交（vs 10-K 的 60-90 天），季度用 6-K 而非 10-Q。FUFU、BTDR、DGHI、SAI 使用 20-F/6-K。
...
| # | Ticker | 公司名称 | 交易所 | SEC Filing | 财年截止月 | IR URL |
|---|--------|---------|--------|-----------|----------|--------|
...
| 15 | SDIG | Stronghold Digital Mining | NASDAQ | 10-K / 10-Q | 12月 | ir.strongholddigitalmining.com |
| 16 | GREE | Greenidge Generation | NASDAQ | 10-K / 10-Q | 12月 | ir.greenidge.com |
| 17 | ABTC | American Bitcoin | NASDAQ | 10-K / 10-Q | 12月 | — |
| 18 | ANY | Sphere 3D | NASDAQ | 10-K / 10-Q | 12月 | sphere3d.com/investors |
| 19 | SLNH | Soluna Holdings | NASDAQ | 10-K / 10-Q | 12月 | solunacomputing.com/investors |
| 20 | AULT | Ault Alliance | NYSE | 10-K / 10-Q | 12月 | aultalliance.com/investors |
| 21 | DGHI | Digihost Technology | NASDAQ | 20-F / 6-K | 12月 | digihost.ca/investors |
| 22 | MIGI | Mawson Infrastructure | NASDAQ | 10-K / 10-Q | **6月** | mawsoninc.com/investors |
| 23 | SAI | SAI.TECH | NASDAQ | 20-F / 6-K | **6月** | saitech.io/investors |
