...
### 失误 5：Adjusted EBITDA 大面积为 null（73% 缺失）

**错误表现**: 94 条财务记录中 69 条 `adjusted_ebitda_usd_m` 为 null。Agent 仅从 10-K/10-Q 正文提取 GAAP 数据，但 Adj. EBITDA 是 **非 GAAP 指标**，不在 10-K/10-Q 正文中。
**根因**: Adjusted EBITDA 出现在**财报新闻稿**中（SEC 8-K 的 Exhibit 99.1），而非 10-K/10-Q 的财务报表部分。所有上市矿企都会在 earnings press release 中披露这个指标。
**正确做法**: 见第 2 章 B 类中新增的 "如何获取 Adjusted EBITDA" 部分。

### 失误 6：9 家公司完全没有财务数据

**错误表现**: HIVE、GREE、ABTC、ANY、SLNH、GPUS、DGXX、MIGI、SAIH 这 9 家公司在 financials.json 中没有任何记录，Agent 声称"找不到数据"。
**根因**: 这 9 家公司**全部仍在上市且有 SEC 报备**。Agent 没有使用正确的 CIK 号码搜索 EDGAR，或者没有考虑到部分公司使用 20-F 而非 10-K。
**正确做法**: 见第 2.5 章"缺失公司 SEC 信息速查"。每家公司都有 CIK 编号和具体的 filing 路径。
...
| Ticker | 财年截止月 | SEC Filing | CIK |
|--------|----------|-----------|-----|
| CLSK | 9月 | 10-K / 10-Q | — |
| IREN | 6月 | 20-F / 6-K | — |
| APLD | 5月 | 10-K / 10-Q | — |
| HIVE | 3月 | **20-F / 6-K** | **1720424** |
| MIGI | 6月 | 10-K / 10-Q | **1218683** |
| SAIH | 6月 | 20-F / 6-K | **1847075** |

其余公司财年截止为 12 月。注意 HIVE 使用 20-F（非 10-K），因为它是加拿大公司（外国私人发行人）。
...
| 2 | SEC EDGAR（按 CIK 搜索） | `https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK={CIK}&type=10-K&dateb=&owner=include&count=10` | 权威。**用 CIK 号码搜索更准确**，见第 2.5 章 |
| 3 | SEC EDGAR 全文搜索 | `https://efts.sec.gov/LATEST/search-index?q={TICKER}&forms=10-K,10-Q,20-F,6-K` | 检查最新 filing |
| 4 | **SEC 8-K（获取 Adj. EBITDA）** | `https://efts.sec.gov/LATEST/search-index?q={TICKER}&forms=8-K` | **Adj. EBITDA 在 8-K Exhibit 99.1 中**，见下方说明 |
| 5 | Stock Analysis | `https://stockanalysis.com/stocks/{TICKER}/financials/?p=quarterly` | 结构化数据 |
| 6 | Yahoo Finance | `https://finance.yahoo.com/quote/{TICKER}/financials` | 备用 |
| 7 | Macrotrends | `https://www.macrotrends.net/stocks/charts/{TICKER}/` | 历史数据补全 |

#### ⚠️ 如何获取 Adjusted EBITDA（重要！）

`adjusted_ebitda_usd_m` 是**非 GAAP 指标**，**不在 10-K/10-Q 的财务报表中**。它出现在以下位置：

**来源 1（最佳）：SEC 8-K → Exhibit 99.1（财报新闻稿）**
```
搜索步骤:
1. 打开 SEC EDGAR: https://efts.sec.gov/LATEST/search-index?q={TICKER}&forms=8-K
2. 找到与 earnings release 相关的 8-K（通常标题含 "Reports Financial Results" 或 "Announces Q* Results"）
3. 下载附件 Exhibit 99.1（即 press release）
4. 在文档中搜索 "Adjusted EBITDA" 或 "Non-GAAP" 或 "Reconciliation"
5. 找到 "GAAP to Non-GAAP Reconciliation" 表格
6. 提取对应季度的 Adjusted EBITDA 数值
```

**来源 2（备选）：公司 IR 页面 → Press Releases**
```
直接访问公司 IR 页面的 Press Releases 或 News 版块，
找到 "Reports Q* Financial Results" 标题的新闻稿，
其中必定包含 Adjusted EBITDA 非 GAAP 调节表。
```

**来源 3（兜底计算）：从 GAAP 数据推算**
```
如果确实无法从 press release 获取，可以用以下公式估算:

Adjusted EBITDA ≈ operating_income + depreciation + stock_based_compensation + impairment

更简化版:
EBITDA ≈ operating_income + depreciation

在 notes 中标注 "EBITDA estimated from GAAP data: operating_income + depreciation"
此方法精度较低，仅在无法获取官方数据时使用。
```

**所有 11 家主要公司都在 earnings press release 中报告 Adj. EBITDA，没有例外。**
...
## 第 2.5 章：缺失公司 SEC 信息速查（必读）

> 以下 9 家公司在之前的采集中完全没有财务数据。**它们全部仍然上市，全部有 SEC 报备。** 这里列出每家公司的精确 SEC 信息，Agent 必须使用这些信息去 EDGAR 搜索。

### SEC EDGAR 搜索方法

**用 CIK 号码搜索比用 Ticker 更准确**（因为部分公司改过名/改过 Ticker）：
...
EDGAR 全文搜索:
https://efts.sec.gov/LATEST/search-index?q=%22{COMPANY_NAME}%22&forms=10-K,10-Q,20-F,6-K,8-K
```

### 9 家缺失公司详情

| # | Ticker | CIK | SEC 名称 | Filing 类型 | 财年截止 | 特殊说明 |
|---|--------|-----|---------|------------|---------|---------|
| 1 | **HIVE** | 1720424 | HIVE Digital Technologies Ltd. | **20-F / 6-K** | **3月** | 加拿大公司，外国私人发行人。20-F 年报约在财年结束后 4 个月提交 |
| 2 | **GREE** | 1844971 | Greenidge Generation Holdings Inc. | 10-K / 10-Q | 12月 | 纽约州天然气发电+挖矿。正常美国公司，直接搜 10-K |
| 3 | **ABTC** | 1755953 | American Bitcoin Corp. | 10-K / 10-Q | 12月 | 原 Gryphon Digital Mining，2025年与 Hut 8 子公司 American Bitcoin 合并。**搜索时用 CIK 1755953 或 "Gryphon Digital"** |
| 4 | **ANY** | 1591956 | Sphere 3D Corp. | 10-K / 10-Q | 12月 | ⚠️ **面临 Nasdaq 退市风险**（2026年3月2日截止期）。仍有 SEC filing，但数据可能不完整 |
| 5 | **SLNH** | 64463 | Soluna Holdings, Inc. | 10-K / 10-Q | 12月 | CIK 号码很小（老公司改名）。搜索用 CIK 64463 |
| 6 | **GPUS** | 896493 | Hyperscale Data, Inc. | 10-K / 10-Q | 12月 | 原 Ault Alliance → BitNile → Hyperscale Data。**搜索时也尝试 "BitNile" 或 "Ault Alliance"** |
| 7 | **DGXX** | 1854368 | Digihost Technology Inc. → Digi Power X | **20-F / 6-K** | 12月 | 加拿大公司，外国私人发行人。2025年3月改名 DGHI→DGXX |
| 8 | **MIGI** | 1218683 | Mawson Infrastructure Group Inc. | 10-K / 10-Q | **6月** | 澳洲背景但在美注册。2024年遭遇非自愿破产申请，已存活。搜索用 CIK 1218683 |
| 9 | **SAIH** | 1847075 | SAI.TECH Global Corp. | **20-F / 6-K** | **6月** | 开曼群岛注册，外国私人发行人。原 ticker SAI，2024年9月改为 SAIH |

### 外国私人发行人（20-F/6-K）采集注意事项

以下 4 家公司使用 **20-F（年报）和 6-K（临时报告）**，而非 10-K/10-Q：

| 公司 | 20-F 提交时间 | 6-K 用途 |
|------|-------------|---------|
| HIVE (CIK 1720424) | 财年结束（3月）后约 4 个月 | 季度更新、月度产量报告 |
| DGXX (CIK 1854368) | 财年结束（12月）后约 4 个月 | 季度更新 |
| SAIH (CIK 1847075) | 财年结束（6月）后约 4 个月 | 季度更新 |
| FUFU (已在主列表) | 财年结束（12月）后约 4 个月 | 季度更新 |
| BTDR (已在主列表) | 财年结束（12月）后约 4 个月 | 季度更新 |
| IREN (已在主列表) | 财年结束（6月）后约 4 个月 | 季度更新 |

```
搜索 20-F:
https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK={CIK}&type=20-F&dateb=&owner=include&count=10

搜索 6-K:
https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK={CIK}&type=6-K&dateb=&owner=include&count=20
```

**关键区别**:
- 20-F = 年度报告（相当于 10-K），提交截止日更长（通常 4 个月 vs 60-90 天）
- 6-K = 临时报告（可包含季度财务、重大事件等，相当于 8-K + 10-Q 的混合体）
- 6-K 中查找季度财务数据时，搜索 "Interim Financial Statements" 或 "Unaudited Financial Results"
- 6-K 中的 Exhibit 99.1 通常包含 earnings press release（含 Adj. EBITDA）

### 改名公司搜索技巧

多家公司经历过改名，用旧名搜索 EDGAR 可能找到更多历史数据：

| 当前 Ticker | 当前名称 | 历史名称/Ticker | EDGAR 搜索建议 |
|------------|---------|---------------|---------------|
| GPUS | Hyperscale Data | Ault Alliance → BitNile Holdings | 搜 CIK 896493，或搜 "BitNile" |
| DGXX | Digi Power X | Digihost Technology (DGHI) | 搜 CIK 1854368 |
| SAIH | SAIHEAT | SAI.TECH Global (SAI) | 搜 CIK 1847075 |
| ABTC | American Bitcoin | Gryphon Digital Mining | 搜 CIK 1755953 或 "Gryphon" |
| MIGI | Mawson Infrastructure | Luna Squares → Mawson | 搜 CIK 1218683 |

### ANY (Sphere 3D) 特殊处理

⚠️ ANY 面临 2026年3月2日的 Nasdaq 合规截止日期。如果在采集时已退市：
- `companies.json`: 将 `active` 设为 `false`
- 历史财务数据仍需补录（SEC filing 仍可访问）
- 在 `notes` 中注明退市状态
...
A: 20-F 通常在财年结束后 4 个月内提交（vs 10-K 的 60-90 天）。使用 20-F/6-K 的公司：FUFU、BTDR、IREN、HIVE、DGXX、SAIH。详见第 2.5 章。

**Q: 用 Ticker 搜 EDGAR 找不到 filing？**
A: 改用 CIK 号码搜索。部分公司改过名（如 GPUS 原名 BitNile、ABTC 原名 Gryphon），用旧 Ticker 搜不到。所有公司的 CIK 号码见第 2.5 章。

**Q: Adjusted EBITDA 在 10-K/10-Q 中找不到？**
A: 这是正常的。Adj. EBITDA 是非 GAAP 指标，在 8-K Exhibit 99.1（earnings press release）中。详见第 2 章 B 类"如何获取 Adjusted EBITDA"。

**Q: 某公司声称"无法找到财务数据"怎么办？**
A: 所有 22 家公司（除 SDIG 已退市）都有 SEC filing。如果搜不到，请：(1) 用 CIK 搜而不是 Ticker；(2) 检查是否搜了正确的 filing 类型（20-F vs 10-K）；(3) 检查公司是否改过名。

---

## 附录：公司速查表

| # | Ticker | CIK | 公司名称 | 交易所 | SEC Filing | 财年截止 | IR URL |
|---|--------|-----|---------|--------|-----------|---------|--------|
...
| 14 | HIVE | **1720424** | HIVE Digital Technologies | NASDAQ | **20-F/6-K** | **3月** | hivedigitaltechnologies.com/investors |
| 15 | GREE | **1844971** | Greenidge Generation | NASDAQ | 10-K/10-Q | 12月 | ir.greenidge.com |
| 16 | ABTC | **1755953** | American Bitcoin (原 Gryphon) | NASDAQ | 10-K/10-Q | 12月 | — |
| 17 | ANY | **1591956** | Sphere 3D ⚠️退市风险 | NASDAQ | 10-K/10-Q | 12月 | sphere3d.com/investors |
| 18 | SLNH | **64463** | Soluna Holdings | NASDAQ | 10-K/10-Q | 12月 | solunacomputing.com/investors |
| 19 | GPUS | **896493** | Hyperscale Data (原 BitNile) | NYSE American | 10-K/10-Q | 12月 | hyperscaledata.com/investors |
| 20 | DGXX | **1854368** | Digi Power X (原 Digihost) | NASDAQ | **20-F/6-K** | 12月 | investors.digipowerx.com |
| 21 | MIGI | **1218683** | Mawson Infrastructure | NASDAQ | 10-K/10-Q | **6月** | mawsoninc.com/investors |
| 22 | SAIH | **1847075** | SAIHEAT (原 SAI.TECH) | NASDAQ | **20-F/6-K** | **6月** | ir.saiheat.com |

**CIK 号码说明**: 加粗的 CIK 号码是之前采集缺失的 9 家公司。Agent 搜索这些公司时**必须使用 CIK 号码**而非 Ticker。其余主要公司用 Ticker 搜索即可。

**Ticker 变更历史**: AULT→GPUS (2024-09), DGHI→DGXX (2025-03), SAI→SAIH (2024-09), SDIG→退市并入 BITF (2025-03), Gryphon→ABTC (2025, 与 Hut 8 子公司合并)

**外国私人发行人（20-F/6-K）**: FUFU, BTDR, IREN, **HIVE**, **DGXX**, **SAIH** — 共 6 家。这些公司不提交 10-K/10-Q，必须搜索 20-F 和 6-K。
